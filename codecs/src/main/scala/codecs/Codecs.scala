...
trait EncoderInstances:
  ...
  Encoder.fromFunction(s => Json.Str(s)) // DONE Implement the `Encoder[String]` given instance

  /** An encoder for `Boolean` values */
  given boolEncoder: Encoder[Boolean] =
  Encoder.fromFunction(b => Json.Bool(b)) // DONE Define a given instance of type `Encoder[Boolean]`
...
trait DecoderInstances:
  ...
  // DONE Define a given instance of type `Decoder[Int]`
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      val maybeDecoded: Option[A] = decoder.decode(item)
      maybeDecoded match
