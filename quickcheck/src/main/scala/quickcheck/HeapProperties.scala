...
trait HeapProperties(val heapInterface: HeapInterface):
    ...
    "melding a heap containing two low values with a heap containing two high values" ->
      forAll { (x: Int, y: Int) =>
        ...
        val meldedHeap: List[Node] =
          val listX = insert(x, insert(x, empty))
          val listY = insert(y, insert(y, empty))
          if x > y then
            meld(listX, listY)
          else
            meld(listY, listX)
        ...
        val deleteTwoMinAndFindMin: Boolean =
          val highest = findMin(deleteMin(deleteMin(meldedHeap)))
          if x > y then highest == x else highest == y
        ...
        if  x > y then y == findMin(insert(y, meldedHeap)) else x == findMin(insert(x, meldedHeap))
  ...
  val meldingHeaps: (String, Prop) =
    ...
    def check(heap1: List[Node], heap2: List[Node], heapMeld: List[Node]): Boolean =
      if isEmpty(heapMeld)
      then isEmpty(heap1) && isEmpty(heap2)
      else
        val minMeld = findMin(heapMeld)
        val trimMeld = deleteMin(heapMeld)
        if isEmpty(heap1)
        then
          findMin(heap2) == minMeld && check(heap1, deleteMin(heap2), trimMeld)
        else
          if isEmpty(heap2)
          then
            findMin(heap1) == minMeld && check(deleteMin(heap1), heap2, trimMeld)
          else
            if findMin(heap2) < findMin(heap1)
            then
              findMin(heap2) == minMeld && check(heap1, deleteMin(heap2), trimMeld)
            else
              findMin(heap1) == minMeld && check(deleteMin(heap1), heap2, trimMeld)
